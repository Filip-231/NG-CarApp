...
class PopularListView (APIView):
    def get (self, request):
        num_popular_cars=2
        cars=Car.objects.order_by('-rating__rates_number')[0:num_popular_cars]
...
def update_rating(car_id, cur_rating):
    ...
    rating.rates_sum = rating.rates_sum+cur_rating
    rating.rates_number = rating.rates_number+1
    rating.save()
